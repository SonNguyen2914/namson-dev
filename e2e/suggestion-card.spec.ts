...
//     target, corners, cards, threat and ONE MATCH STATE PER SIDE
...
//
// THE SAME MISTAKE, TWICE, AND THE THIRD TIME IS `states`.
//
// `tilt_label` sat here as the bare string `"SIEGE"` with a `tilt_note`
// sibling until 2026-08-21 — a shape card.py `_tilt` had no branch to
// return — so this suite was green while every real in-play card threw
// React #31 and blanked. `threat` was the same thing one field over: a
// bare `0.72` until 2026-08-24, while `_live_state` had emitted
// `{tilt, fav, basis}` since the block shipped.
//
// AND `tilt_label` ITSELF IS NOW A SHAPE THE BACKEND CANNOT EMIT.
// Backend `c3044d1` replaced it with `states` on 2026-09-07 — one word
// per SIDE, needing no favourite — and this fixture kept the dead key
// until 2026-09-08, so the suite went on certifying a chip that could
// never draw while the real payload's `states` was being named on
// screen as an unaccounted key. The lesson is the one the two comments
// above already record, so the fixture below is RECORDED OFF THE
// EMITTER: `card.live_states` called on a row carrying exactly the
// possession, shots and score printed beside it, not typed out by hand.
const STATE_CUTS_ARE_CONVENTIONS = "EXPLORATORY — NOT VALIDATED: the "
  + "share>=0.6 / share<=0.4 cut is a DECLARED CONVENTION and not a "
  + "fitted threshold. Nothing was preregistered, no holdout was held "
  + "out, and no measured edge is attached to any of these six words. "
  + "AND THE TWO AXES ARE NOT EQUALLY UNFITTABLE, which is the half a "
  + "reader would otherwise supply wrongly: the CHANCES cut COULD be "
  + "fitted, because M1's panel carries shot volume at four horizons "
  + "and measured it; the BALL cut could NOT, because possession is "
  + "not in that corpus at all (M1's own `possession_not_on_tape` — "
  + "refused rather than tested — and src/live/live_rates.py carries a "
  + "guard that it never reads a possession column). One cut is "
  + "unmeasured; the other is unmeasurable on the evidence this "
  + "project holds. Neither may be read as the measured one. The words "
  + "describe THIS row; they forecast nothing.";

const STATE_MISSING_IS_NEVER_EVEN = "MISSING IS NEVER AN EVEN MATCH. A "
  + "row without possession or without shots for BOTH sides has NO "
  + "state — not CONTEST, which would claim the two sides are level, "
  + "and not a zero share, which would claim one of them has done "
  + "nothing. The tape's own rule (src/live/models.py "
  + "LiveStatSnapshot: missing is never zero), read in the direction a "
  + "label makes it easy to break.";

const SIEGE_NOTE = "this side has the ball AND the chances (ball · "
  + "chances). " + STATE_CUTS_ARE_CONVENTIONS;
const PINNED_NOTE = "neither the ball nor the chances, and this side "
  + "is not ahead (no ball · no chances · behind). "
  + STATE_CUTS_ARE_CONVENTIONS;

/** `card.live_states` on possession 61.4/38.6, shots 14/6, score 1-0 —
 *  the very counts LIVE_STATE prints beside it, so the two words and
 *  the bars above them come off ONE set of numbers (AGENTS.md §3).
 *  Home 0.614 and 0.700 are both at or above the cut: SIEGE. Away is
 *  0.386 and 0.300, both at or below it, and BEHIND — which is the one
 *  cell the score splits, and the reason it reads PINNED and not LOW
 *  BLOCK. Trimmed to the keys this surface reads plus the two the
 *  refusal shape shares; nothing is REWORDED. */
const STATES_SIEGE = {
  home: {
    state: "SIEGE",
    cell: { ball: "has", chances: "has", score: "ahead",
            score_decides: false },
    cell_words: "ball · chances",
    shares: { ball: 0.614, chances: 0.7 },
    note: SIEGE_NOTE,
    conventions: STATE_CUTS_ARE_CONVENTIONS,
  },
  away: {
    state: "PINNED",
    cell: { ball: "against", chances: "against", score: "behind",
            score_decides: true },
    cell_words: "no ball · no chances · behind",
    shares: { ball: 0.386, chances: 0.3 },
    note: PINNED_NOTE,
    conventions: STATE_CUTS_ARE_CONVENTIONS,
  },
  counts: { possession: { home: 61.4, away: 38.6 },
            shots: { home: 14, away: 6 } },
  cut: { has_at_or_above: 0.6, against_at_or_below: 0.4 },
  vocabulary: ["BLUNT", "CONTEST", "COUNTER", "LOW BLOCK", "PINNED",
               "SIEGE"],
  conventions: STATE_CUTS_ARE_CONVENTIONS,
  shows_not_decides: "A STATE IS A DESCRIPTION OF THIS ROW AND "
    + "FORECASTS NOTHING. It shows; it does not decide.",
};

/** The same emitter on the GAPS row — no possession either side, no
 *  away shot count. BOTH names that fired, and the sentence saying a
 *  row without them has NO state rather than an even one. */
const STATES_REFUSED = {
  refused: "no_possession: the tape carries no possession and no shot "
    + "counts for one or both sides, so neither side has a state. "
    + STATE_MISSING_IS_NEVER_EVEN,
  refusal_codes: ["no_possession", "no_shot_evidence"],
  basis: STATE_MISSING_IS_NEVER_EVEN,
};
...
test.describe("suggestion card (recorded payloads)", () => {
  ...
  test("live_now.state renders the tape's own counts — possession as a "
    + "share, shots / on target / corners / cards as pairs, and one "
    + "match state per side", async ({ page }) => {
      await serveMatch(page);
      ...
      // ONE WORD PER SIDE, both of them, and the cell that fired each.
      // The pair is the point: the label this replaced was computed on
      // the FAVOURITE alone, so an opponent holding the chances got
      // "neither side is tilting" — a sentence about a match that was
      // not on the row.
      const states = page.getByTestId("live-states");
      await expect(states).toHaveAttribute("data-home", "SIEGE");
      await expect(states).toHaveAttribute("data-away", "PINNED");
      await expect(states.getByTestId("live-states-home"))
        .toHaveText("SIEGE");
      await expect(states.getByTestId("live-states-away"))
        .toHaveText("PINNED");
      // the sub-line is the backend's own `cell_words`, which is what
      // lets a reader check the word against the bars above it without
      // taking this block's arithmetic on trust
      await expect(states).toContainText("ball · chances");
      await expect(states).toContainText("no ball · no chances · behind");
      // and the note that says the cut is a DECLARED CONVENTION travels
      // with the word rather than being left behind by it
      expect(await states.getByTestId("live-states-home")
        .getAttribute("title")).toBe(SIEGE_NOTE);
  ...
  // THE EM-DASH BUG, pinned. A computed threat reached the card as
  // `{tilt, fav, basis}` and the component asked whether it was a
  // number, so it printed — beside a confident SIEGE word on every
  // real in-play fixture. The assertion that matters is the PAIRING:
  // the number and the state word on screen together. A dash there is
  // not a cosmetic miss — a readout showing nothing next to a word
  // like SIEGE reads as broken, and a card that reads as broken gets
  // ignored, or gets its word believed on its own.
  test("a computed threat renders as its NUMBER beside the state word, "
    + "with the side it is a share of and the definition reachable",
    async ({ page }) => {
      await serveMatch(page);
      await serveCard(page, withLiveNow(withState(LIVE_NOW, LIVE_STATE)));
      await page.goto(`/bet-suggester/mls/${EVENT}`);
      ...
      // the pairing: the number and the state word sit in the same
      // readout, which is precisely what read as broken while one was a
      // dash
      const state = page.getByTestId("live-state");
      await expect(state).toContainText("0.76");
      await expect(state.getByTestId("live-states-home"))
        .toHaveText("SIEGE");
    });
  ...
  test("a stat the tape does not carry renders \u2014, never 0 — and a "
    + "real 0 still renders 0", async ({ page }) => {
      ...
      // ...and the state block is PRESENT and REFUSED, which is not the
      // same thing as absent: a row with no possession has NO state,
      // and the two names that fired say which inputs it wanted. A
      // CONTEST here would claim the two sides are level.
      const states = page.getByTestId("live-states");
      await expect(states).toHaveAttribute("data-state", "absent");
      // the LEADING code and the sentence, which is what the emitter
      // puts in `refused`; the full `refusal_codes` list rides on the
      // payload and neither this surface nor the live card draws it.
      await expect(states).toContainText("no_possession");
      await expect(states).toContainText("no shot counts");
      await expect(states).toContainText("MISSING IS NEVER AN EVEN MATCH");
      // and NO word is drawn — not CONTEST, which would claim the two
      // sides are level on a row that carries neither share. (The
      // sentence above names CONTEST to say it is not one; the absence
      // that matters is the WORD, so it is asserted on the word.)
      await expect(page.getByTestId("live-states-home")).toHaveCount(0);
      await expect(page.getByTestId("live-states-away")).toHaveCount(0);
    });
...
test.describe("the ladder: exposure, tick age, and the position", () => {
  ...
  test("a REFUSED state renders its words and the card survives — the "
    + "block is an object, not a word", async ({ page }) => {
      // REGRESSION, found by rendering the backend's real bytes rather
      // than a hand-written stand-in. The key this replaces,
      // `tilt_label`, returned `{label, note}` or `{refused}` and had NO
      // string branch, so a string-typed renderer threw React #31 and
      // the whole card became a client-side exception — on every
      // in-play fixture, while this suite stayed green against a shape
      // nothing emits. `states` refuses the same way and is pinned the
      // same way.
      const errors: string[] = [];
      page.on("pageerror", (e) => errors.push(String(e)));
      await serveMatch(page);
      await serveCard(page, withLadder({
        live: withState(LIVE_NOW, LIVE_STATE_GAPS) }));
      await page.goto(`/bet-suggester/mls/${EVENT}`);
      ...
      // the refusal renders in the emitter's own words, and no word is
      // invented to stand in for a pair that was declined
      await expect(page.getByTestId("live-states"))
        .toContainText("no_possession");
      await expect(page.getByTestId("live-states-home")).toHaveCount(0);
    });

  test("a state that HAS both words renders them with their cells, and "
    + "throws nothing", async ({ page }) => {
      ...
      await serveCard(page, withLadder());
      await page.goto(`/bet-suggester/mls/${EVENT}`);
      await expect(page.getByTestId("live-states-home"))
        .toHaveText("SIEGE");
      await expect(page.getByTestId("live-states-away"))
        .toHaveText("PINNED");
      await expect(page.getByTestId("live-states"))
        .toContainText("no ball · no chances · behind");
      expect(errors).toEqual([]);
    });
...
test.describe("a failed read is not an empty one", () => {
  ...
  // THE OTHER DIRECTION OF THE SAME GUARD, and the one that was open.
  //
  // The namer is only worth having if it fires on a key nobody
  // declared. It is only worth TRUSTING if it stays silent on a payload
  // shaped the way the backend actually sends one — and it did not.
  // `states` replaced `tilt_label` in backend `c3044d1`, this file's
  // declared list kept the two dead names and never gained the live
  // one, and so from 2026-09-07 EVERY in-play card carried an amber
  // "this surface has no recorded shape for `states`" warning while the
  // sibling live card was drawing that very block. A warning that fires
  // on every card is a warning nobody reads.
  //
  // THE KEY LIST IS THE EMITTER'S, recorded 2026-09-08 by calling
  // `card._live_state(row, "home")` and sorting its keys — not copied
  // out of LIVE_STATE above, because a fixture that has drifted from
  // the wire is precisely what this catches.
  const LIVE_STATE_KEYS_ON_THE_WIRE = [
    "basis", "cards", "corners", "on_target", "possession", "shots",
    "states", "threat", "window",
  ];

  // the two keys LIVE_STATE does not carry, RECORDED off the same call.
  // A stand-in of the wrong TYPE would be its own defect: `basis` is a
  // string on the wire, and an object rendered as a React child is the
  // #31 blank card this file has already shipped once.
  const LIVE_STATE_BASIS = "the same state-tape row the triple above "
    + "was read from — no second query and no second provider call, so "
    + "the numbers and the state can never describe two different "
    + "minutes. A NULL is the provider's silence and renders as "
    + "unknown: missing is never zero (src/live/models.py "
    + "LiveStatSnapshot).";
  const NO_TAPE_HANDED = "no_tape_handed: the caller passed no tape at "
    + "all, so there is no earlier row to difference against. This is "
    + "not a failed read (that is tape_unreadable) and not a tape "
    + "without an old enough row (that is no_earlier_row): it is a "
    + "reader that never asked";
  const LIVE_STATE_WINDOW = {
    measured_over: { refused: NO_TAPE_HANDED },
    tilt: { refused: NO_TAPE_HANDED },
    event_density: { refused: NO_TAPE_HANDED },
    target_minutes: 15,
  };
  const RECORDED_ELSEWHERE: Record<string, unknown> = {
    basis: LIVE_STATE_BASIS, window: LIVE_STATE_WINDOW,
  };

  test("a live state carrying EVERY key the backend emits raises no "
    + "unaccounted-key warning at all", async ({ page }) => {
      // built FROM the recorded key list, so a name added to it that
      // the component never declared fails here
      const asEmitted: Record<string, unknown> = {};
      for (const k of LIVE_STATE_KEYS_ON_THE_WIRE) {
        asEmitted[k] = (LIVE_STATE as Record<string, unknown>)[k]
          ?? RECORDED_ELSEWHERE[k];
        expect(asEmitted[k], `${k} has no recorded value`)
          .not.toBeUndefined();
      }
      await serveMatch(page);
      await serveCard(page, withLiveNow(withState(LIVE_NOW, asEmitted)));
      await page.goto(`/bet-suggester/mls/${EVENT}`);

      // the block DID render — without this the assertion below would
      // pass against a card that drew no state at all
      await expect(page.getByTestId("live-state")).toBeVisible();
      await expect(page.getByTestId("live-states-home"))
        .toHaveText("SIEGE");

      // and not one key on it was named as unrecorded
      await expect(page.getByTestId("live-state-unaccounted-key"))
        .toHaveCount(0);
      await expect(page.getByTestId("live-state-unaccounted"))
        .toHaveCount(0);
      // said again on the key that actually drifted, so a failure names
      // it rather than leaving a reader to diff two lists
      await expect(page.locator('[data-key="states"]')).toHaveCount(0);
    });
