...
// different: epl-2026-v0 is DARK — unapproved, no runs exist, the
// backend structurally refuses them — so every model slot renders its
// explicit no-prediction state. Nothing here fakes a number, and if a
// run ever appears it is labeled shadow / not advice exactly as MLS.
// Result letters are derived from the scores beside them server-side
// (ESPN's raw `score` string is winner-first — never rendered).
...
export default function EplMatchPage() {
  ...
  // once a canonical T-10 lock exists it IS the fixture's model — a
  // later scheduled run must never silently supersede it (V8 eval F9)
    ...
    <div style={EPL_VARS} className="min-h-screen bg-bs font-sans text-ink-mid">
      <Head><title>
        {m ? (m.home.score != null && m.away.score != null
          ? `${m.home.abbrev} ${m.home.score}–${m.away.score} ${m.away.abbrev} · EPL`
      ...
      <div className="mx-auto max-w-2xl px-4 py-10">
        ...
        {m && (
          <>
            ...
            {/* ===== team news: announced XI (no strength column — no
                 per-player stat source exists for the EPL) ===== */}
            <LineupSection lu={lineups} m={m} run={run} />

            {/* ===== ESPN scouting: form + H2H ===== */}
            <ScoutingSection m={m} />

            {/* ===== market vs model — aligned three-way bars ===== */}
                ...
                <div className="rounded-2xl border border-line bg-elev p-5">
                  ...
                  <div className="mt-5 border-t border-line pt-4">
                    ...
                    <TripleBar m={m} probs={modelProbs(run)}
                      caption={run ? `epl-2026-v0 · ${run.n_simulations?.toLocaleString()} sims · seed ${run.seed}${run.run_type === "t10" ? " · T-10 LOCK — frozen pre-kickoff" : ""}` : undefined}
                      emptyText="model dark — no approved prediction run exists" />
                  </div>
                  <p className="mt-4 font-mono text-[9px] uppercase leading-relaxed tracking-[0.12em] text-ink-faint">
                    epl-2026-v0 must earn shadow approval through the
                    evaluation ladder on real 2026-27 results before any
                    probability appears here · real-money signals stay
            ...
            {/* ===== model prediction: only if a run ever exists ===== */}
            ...
            {/* ===== scenario engine (pure execution arithmetic) ===== */}
            <Reveal>
            ...
            <p className="mt-12 text-center font-mono text-[10px] uppercase tracking-[0.15em] text-ink-faint">
              ...
              then shadow-only · not betting advice
            </p>
          </>
        )}
      </div>
...
function TripleBar({ m, probs, caption, emptyText }: {
  ...
  const hc = sideColor(m.home, HOME_FALLBACK);
  const ac = sideColor(m.away, AWAY_FALLBACK);
  return (
    <div className="mt-3">
...
const COLLAPSED_FAMILIES = new Set(
  ["score", "h1", "h1_total", "h1_spread", "h1_btts"]);
    ...
    <Reveal>
      <div className="mt-4">
        ...
        <div className="overflow-x-auto rounded-xl border border-line">
          <div className="min-w-[560px]">
            <div className="grid grid-cols-[minmax(0,1fr)_5.5rem_5rem_5.5rem_5.5rem] items-center gap-x-3 border-b border-line bg-elev px-4 py-3 font-mono text-[11px] uppercase tracking-[0.14em] text-ink-low">
              <span>Market</span>
              <span className="text-right"
                title="epl-2026-v0 shadow probability — empty while the model is dark">Likelihood</span>
              <span className="text-right"
                title="MODEL probability minus the CURRENT ask minus Kalshi's entry fee — empty while the model is dark">Net edge</span>
              <span className="text-right"
            ...
            {fams.map((f) => {
              ...
              return (
                <div key={f.key}>
                  ...
                  {!fold && rowsFor(f).map((r) => {
                    const edge = r.modelP != null && r.ask != null
                      ? r.modelP - (r.ask + fee(r.ask)) : null;
                    return (
                      <div key={r.ticker}
                        ...
                        <span className="min-w-0 truncate pr-2 text-ink-hi" title={r.ticker}>
                          {r.label}
                        </span>
                        <span className="text-right font-mono tabular-nums text-ink-hi">
                          {r.modelP != null ? pct(r.modelP) : "—"}
                        </span>
                        ...
                        <span className="text-right font-mono tabular-nums text-ink-mid">
                          {r.ask != null && r.ask > 0 ? `${(1 / r.ask).toFixed(2)}x` : "—"}
                        </span>
                        <span className="text-right font-mono tabular-nums text-ink-mid">
                          {r.ask != null ? `${Math.round(r.ask * 100)}¢` : "—"}
                          <span className="text-ink-faint">
                            {r.bid != null ? ` / ${Math.round(r.bid * 100)}¢` : ""}
                          </span>
                        </span>
                      </div>
                    );
        ...
        <p className="mt-2 font-mono text-[9px] uppercase leading-relaxed tracking-[0.12em] text-ink-faint">
          likelihood = a stored shadow run&apos;s probability where one
          exists — while epl-2026-v0 is dark every model cell shows
          &quot;—&quot; and only the exchange&apos;s own prices render ·
          shadow, not advice
        </p>
      </div>
    </Reveal>
    ...
    <div>
      ...
      <div className="flex h-1 gap-0.5 overflow-hidden rounded-full">
        <div className="rounded-full"
          style={{ width: `${hw}%`,
            background: sideColor(m.home, HOME_FALLBACK) }} />
...
/* ---------- team news: announced XI ----------
   DISPLAY CONTEXT ONLY, and unlike MLS with NO strength column: no
   per-player stat source exists for the EPL (no Sportec analogue), so
   the xG/90 annotation is honestly absent rather than approximated. */
...
function LineupSection({ lu, m, run }: {
  ...
  return (
    <Reveal>
      <Collapse eyebrow="team news" title="lineups"
        ...
        <div className="mb-3 rounded-xl border border-line bg-elev2 px-3 py-2">
          <p className="font-mono text-[9px] uppercase tracking-[0.14em] text-ink-low">
            ...
            now{run ? ", not the lineup frozen at T-10" : ""}
          </p>
        </div>
            ...
            <div className="mt-4 rounded-2xl border border-line p-4">
              ...
              <div className="space-y-1.5">
                {sc.head_to_head.map((g, i) => {
                  ...
                  // order; reorder to perspective-first so W/L/D always
                  // agrees with the numbers the eye reads
                  const away = g.at_vs === "@";
