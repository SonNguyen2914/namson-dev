...
// La Liga match hub — the MLS skeleton (Son's Jul 23 layout) rewired
// for esp.1 + KXLALIGA*, model DARK: the compact match-info card
...
export default function LaLigaMatchPage() {
  ...
  useEffect(() => {
    ...
    let alive = true;
    const load = () =>
      fetch(`/api/laliga/match/${eventId}`)
  ...
  return (
    <div style={LALIGA_VARS} className="min-h-screen bg-bs font-sans text-ink-mid">
      <Head><title>
        {m ? (m.home.score != null && m.away.score != null
          ? `${m.home.abbrev} ${m.home.score}–${m.away.score} ${m.away.abbrev} · La Liga`
      ...
      <div className="mx-auto max-w-2xl px-4 py-10">
        ...
        {m && (
          <>
            ...
            {eventId && (
              <SuggestionCard key={eventId} competition="la-liga-2026"
                eventId={eventId} />
            )}
                      ...
                      <div className="flex h-1.5 gap-0.5 overflow-hidden rounded-full">
                        <div className="rounded-full"
                          style={{
                            ...
                            background: sideColor(m.home, "#ff4b44"),
                          }} />
                ...
                <div className="rounded-2xl border border-line bg-elev p-5">
                  ...
                  <div className="mt-5 border-t border-line pt-4">
                    ...
                    <TripleBar m={m} probs={modelProbs(run)}
                      caption={run ? `laliga-2026-v0 · ${run.n_simulations?.toLocaleString()} sims · seed ${run.seed}${run.run_type === "t10" ? " · T-10 LOCK — frozen pre-kickoff" : ""}` : undefined}
                      emptyText="laliga-2026-v0 is dark — no approved model, so no prediction exists" />
                  </div>
...
function TripleBar({ m, probs, caption, emptyText }: {
  ...
  const hc = sideColor(m.home, "#ff4b44");
        ...
        <div className="overflow-x-auto rounded-xl border border-line">
          <div className="min-w-[560px]">
            <div className="grid grid-cols-[minmax(0,1fr)_5.5rem_5rem_5.5rem_5.5rem] items-center gap-x-3 border-b border-line bg-elev px-4 py-3 font-mono text-[11px] uppercase tracking-[0.14em] text-ink-low">
              <span>Market</span>
              <span className="text-right" title="laliga-2026-v0 shadow probability">Likelihood</span>
              <span className="text-right"
    ...
    <Reveal>
      <Collapse eyebrow="team news" title="lineups + absentees"
        ...
        <p className="mt-3 font-mono text-[9px] uppercase tracking-[0.14em] text-ink-faint">
          xi from espn · xg/90 unavailable — no official la liga stats feed ·
